...
pattern = re.compile(r"    def apply_diff_highlight_colors\(self,\r?\n(?:        .+\r?\n){0,200}?        except Exception as e:\r?\n            raise ToolExecutionError\(f\"差分ハイライトの色適用中にエラーが発生しました: \{e\}\"\) from e\r?\n", re.DOTALL)
...
new_block = """    def apply_diff_highlight_colors(self,
        ...
        \"\"\"Apply highlight colors to partial text spans inside a cell.\"\"\"
        try:
            if not style_matrix:
            ...
            for r_idx in range(max_rows):
                ...
                if not isinstance(row_styles, list):
                    _diff_debug(f\"apply_diff_highlight_colors row {r_idx} has invalid style entry\")
                    continue
                ...
                for c_idx in range(max_cols):
                    ...
                    if not isinstance(cell_value, str):
                        ...
                        continue
                    _diff_debug(
                        f\"apply_diff_highlight_colors cell({r_idx},{c_idx}) spans={spans} value={_shorten_debug(cell_value)}\"
                    )
                    for span in spans:
                        if not isinstance(span, dict):
                            _diff_debug(f\"apply_diff_highlight_colors cell({r_idx},{c_idx}) span not dict: {span}\")
                            continue
                        start = span.get(\"start\")
                        length = span.get(\"length\")
                        span_type = span.get(\"type\")
                        if start is None or length is None or length <= 0:
                            _diff_debug(f\"apply_diff_highlight_colors cell({r_idx},{c_idx}) invalid span bounds {span}\")
                            continue
                        if span_type == \"追加\":
                        ...
                        if color_value is None:
                            _diff_debug(f\"apply_diff_highlight_colors cell({r_idx},{c_idx}) unknown span type {span_type}\")
                            continue
                        try:
