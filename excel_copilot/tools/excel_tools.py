    ...
    try:
        ...
        def _extract_source_sentences_batch(
            ...
            source_sentence_prompt_sections: List[str] = [
                "目的: items(JSON) に含まれる各 source_text を裏付ける日本語の引用文を、提供された参照URLから抽出してください。",
                "",
                "指示:",
                "1. context_id ごとに 1〜5 件の短い文を候補として返し、見つからない場合は空配列 [] にしてください。",
                "2. 文は参照URLに実際に書かれている内容をそのまま引用し、source_text と同一の文や推測で生成した文は含めないでください。",
                "3. 参照URLと無関係な情報や脚注番号 ([1] など)・装飾記号は取り除いてください。",
                "4. ほかの context_id と極力重複しないようにしながら、必要なら最大 10 件まで検討してください。",
                "5. URL周辺から適切な文が見つからない場合は無理に作成せず、その context_id は空配列のままにしてください。",
